...
class UI:
    ...
    def message(self, message, go_back_to):
        ...
        tkt.Button(message_frame, text="Got it!", command=go_back_to, font=self.button_font, bg="grey",
                   fg="black").pack()
    ...
    def customer_window(self):
        ...
        greeting_label = tkt.Label(main_customer_frame, text=f'Greetings! {self.customer.username}', bg="navyblue", fg="lightblue", font=("Helvetica", 30, "bold"))
        greeting_label.pack(side="top", anchor="nw")

        # Create a frame for the table and images
        menu_frame = tkt.Frame(main_customer_frame, bg="#e6f7ff")
        ...
        for idx, heading in enumerate(headings):
            tkt.Label(menu_frame, text=heading, bg="#e6f7ff", font=self.label_font).grid(row=0, column=idx, padx=10, pady=5)

        # Load the menu from the CSV file
        self.load_menu_from_csv(menu_frame)

        # Add buttons for cart management and order placement
        tkt.Button(main_customer_frame, text="View Cart", command=self.view_cart, font=self.button_font, bg="grey", fg="black").pack(pady=20)
    ...
    def add_menu_item(self, item, parent_frame, row_index):
        ...
        image_label = tkt.Label(parent_frame, image=image_tk, bg="#e6f7ff")
        ...
        inc_button = tkt.Button(parent_frame, text='+', command=lambda item=item: self.update_quantity(item, quantity_var, 1))
        dec_button = tkt.Button(parent_frame, text='-', command=lambda item=item: self.update_quantity(item, quantity_var, -1))
        quantity_label = tkt.Label(parent_frame, textvariable=quantity_var, bg="#e6f7ff", font=self.label_font)

        # Place the elements in the grid
        tkt.Label(parent_frame, text=item_name, bg="#e6f7ff", font=self.label_font).grid(row=row_index, column=0, padx=10, pady=5)  # Item name
        image_label.grid(row=row_index, column=1, padx=10, pady=5)  # Image
        tkt.Label(parent_frame, text=f"₹{item_price:.2f}", bg="#e6f7ff", font=self.label_font).grid(row=row_index, column=2, padx=10, pady=5)  # Price
    ...
    def update_quantity(self, item, quantity_var, delta):
        ...
        if new_quantity >= 0:  # Prevent negative quantities
            ...
            if delta > 0:
                ...
                self.customer.remove_item(self.menu, item['Name'])
        ...
        #
        # #item to be ordered
        # tkt.Label(main_customer_frame, text="What item would you wish to order?(Enter as written in Menu)",
        ...
        # tkt.Label(main_customer_frame, text="How much quantity would you like to order?", bg="#fffff0",
        ...
        #
        # #Buttons
        # tkt.Button(main_customer_frame, text="Add item to Cart", command=self.add_to_cart, font=self.button_font,
        #            bg="grey", fg="black").pack()
        # tkt.Button(main_customer_frame, text="View Cart", command=self.view_cart, font=self.button_font, bg="grey",
        #            fg="black").pack()
        # tkt.Button(main_customer_frame, text="Remove item from Cart", command=self.remove_from_cart,
        #            font=self.button_font, bg="grey", fg="black").pack()
        # tkt.Button(main_customer_frame, text="Order History", command=self.order_history, font=self.button_font,
        #            bg="grey", fg="black").pack()
        # tkt.Button(main_customer_frame, text="Place Order-->", command=self.place_order, font=("Helvetica", 30), bg="lightblue",
        #            fg="navyblue").place(relx=1.0, rely=1.0, anchor='se')
        # tkt.Button(main_customer_frame, text="Logout", command=self.customer_logout, font=("Helvetica", 30), bg="red", fg="white").place(relx=0, rely=1.0, anchor='sw')
        # self.add_footer()
    ...
    def order_history(self):
        ...
        self.message(
            previous_orders_display if previous_orders_display != "" else "Please order something which will show up here",
            self.customer_window)
    ...
    def place_order(self):
        ...
        if total_amount == 0:
            ...
            tkt.Label(place_order_frame,
                      ...
                      bg="#fffff0", font=self.label_font).pack()
            tkt.Button(place_order_frame, text="Yes", command=self.order_placement, font=self.button_font, bg="grey",
                       fg="black").pack()
            tkt.Button(place_order_frame, text="No", command=self.customer_window, font=self.button_font, bg="grey",
                       fg="black").pack()
    ...
    def customer_logout(self):
        ...
        tkt.Button(logout_frame, text="No", command=self.customer_window, font=self.button_font, bg="grey",
                   fg="black").pack()
